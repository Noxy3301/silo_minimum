...
struct Customer {
    ...
    struct Key {
        ...
        std::string pretty_str() const {
            ...
            ::snprintf(buf, sizeof(buf),
                       "Customer_Key: w_id %u  d_id %u  c_id %u", w_id, d_id,
                       c_id);
    ...
    struct CLastKey {
        ...
        /**
         * CAUSION: c_last is just a pointer. Set it approprieately before
         * calling parse().
         */
        size_t parse(const char *in) {
    ...
    // key size is 8 bytes.
    static void CreateKey(uint16_t w_id, uint8_t d_id, uint32_t c_id,
                          char *out) {
...
struct History {
    ...
    // C_ID) (H_W_ID, H_D_ID) Foreign Key, references (D_W_ID, D_ID)
...
struct Item {
    alignas(CACHE_LINE_SIZE) std::uint32_t I_ID; // 200,000 unique IDs 100,000 items are populated
...
struct Stock {
    ...
    alignas(CACHE_LINE_SIZE) std::uint32_t
