...
template <typename Tuple, typename Param> class TPCCInitializer {
public:
    static void db_insert_raw([[maybe_unused]] size_t thid, [[maybe_unused]] Param *param, Storage st, std::string key, void* val) {
        ...
        Table[get_storage(st)].put(key, tuple, 1);
        ...
        //               << ", val : " <<
        //               str_view_hex(rec->get_tuple().get_val()) << std::endl;
        //     std::abort();
        // }
    }
    ...
    static void load_item([[maybe_unused]] Param *param) {
        ...
        struct S {
            static void work([[maybe_unused]] Param *param, std::uint32_t i_id_start, std::uint32_t i_id_end, const IsOriginal &is_original) {
                for (std::uint32_t i_id = i_id_start; i_id <= i_id_end; ++i_id) {
                    ...
                    if (is_original[i_id - 1]) {
                        make_original(ite.I_DATA, dataLen);
                    }
#ifdef DEBUG
                    std::cout << "I_ID:" << ite.I_ID
                              << "\tI_IM_ID:" << ite.I_IM_ID
                              << "\tI_NAME:" << ite.I_NAME
                              << "\tI_PRICE:" << ite.I_PRICE
    ...
    for (auto &&th : thv) {
      th.join();
        ...
        // single threaded.
        //::printf("load item...\n");
        S::work(param, 1, MAX_ITEMS, is_original);  // Structにしているのはnamespaceを汚したくないから？
    ...
    static void load_warehouse([[maybe_unused]] Param *param,
        ...
        make_address(ware.W_STREET_1, ware.W_STREET_2, ware.W_CITY,
                     ware.W_STATE, ware.W_ZIP);
    ...
    static void load_stock([[maybe_unused]] Param *param, std::uint16_t w_id) {
        ...
        struct S {
            static void work([[maybe_unused]] Param *param,
                             std::uint32_t i_id_start, std::uint32_t i_id_end,
                             std::uint16_t w_id,
                             const IsOriginal &is_original) {
                for (std::uint32_t i_id = i_id_start; i_id <= i_id_end;
                     ++i_id) {
                    assert(i_id != 0); // 1-origin
                    HeapObject obj;
                    obj.allocate<Stock>();
                    ...
                    for (char *out : {st.S_DIST_01, st.S_DIST_02, st.S_DIST_03,
                                      st.S_DIST_04, st.S_DIST_05, st.S_DIST_06,
                                      st.S_DIST_07, st.S_DIST_08, st.S_DIST_09,
                                      st.S_DIST_10}) {
                    ...
                    st.S_REMOTE_CNT = 0;
                    std::size_t dataLen =
                        random_alpha_string(26, 50, st.S_DATA);
                    if (is_original[i_id - 1])
                        make_original(st.S_DATA, dataLen);
    ...
    static void load_orderline([[maybe_unused]] Param *param,
                               std::uint16_t w_id, std::uint16_t d_id,
                               std::uint32_t o_id, uint8_t ol_num) {
        std::time_t now = get_lightweight_timestamp();
        HeapObject obj;
        ...
        db_insert_raw(w_id, param, Storage::OrderLine, key.view(),
                      std::move(obj));
    }

    // CREATE Order
    static void load_order([[maybe_unused]] Param *param, std::uint16_t w_id,
                           uint8_t d_id, std::uint32_t o_id,
                           std::uint32_t c_id) {
        std::time_t now = get_lightweight_timestamp();
        HeapObject obj;
        obj.allocate<Order>();
...

        {
            ...
            db_insert_raw(w_id, param, Storage::Order, key.view(),
                          std::move(obj));
        }
    ...
    static void load_customer([[maybe_unused]] Param *param, uint8_t d_id,
        ...
        struct S {
            static void work([[maybe_unused]] Param *param,
                             std::uint32_t c_id_start, std::uint32_t c_id_end,
                             HistoryKeyGenerator &hkg, uint8_t d_id,
                             std::uint16_t w_id, const Permutation &perm) {
                for (std::uint32_t c_id = c_id_start; c_id <= c_id_end;
                     ++c_id) {
                    ...
                    if (c_id <= 1000) {
                        ...
                        make_c_last(non_uniform_random<255, true>(0, 999),
                                    customer.C_LAST);
                    }
                    copy_cstr(customer.C_MIDDLE, "OE",
                              sizeof(customer.C_MIDDLE));
                    random_alpha_string(8, 16, customer.C_FIRST);
                    make_address(customer.C_STREET_1, customer.C_STREET_2,
                                 customer.C_CITY, customer.C_STATE,
                                 customer.C_ZIP);
                    random_number_string(16, 16, customer.C_PHONE);

#ifdef DEBUG
                    if (c == start && w == 1 && d == 2)
                        std::cout << "C_PHONE:" << customer.C_PHONE
                                  << std::endl;
#endif
                    ...
                    MasstreeWrapper<Tuple>::thread_init(cached_sched_getcpu());
                    Tuple *tuple =
                        Masstrees[get_storage(Storage::CustomerSecondary)]
                            .get_value(c_last_key);
                    if (tuple != nullptr) {
                        memcpy(&ctn_ptr, tuple->body_.get_val().data(),
                               sizeof(uintptr_t));
                        //::printf("found %p\n", ctn_ptr);
                        ...
                        uintptr_t &p = obj.ref();
                        p = uintptr_t(ctn_ptr);
                        db_insert_raw(w_id, param, Storage::CustomerSecondary,
                                      c_last_key, std::move(obj));
                    }

                    struct S {
                        static Customer *search(const SimpleKey<8> &pkey) {
                            MasstreeWrapper<Tuple>::thread_init(
                                cached_sched_getcpu());
                            auto *tuple = reinterpret_cast<Tuple *>(
                                Masstrees[get_storage(Storage::Customer)]
                                    .get_value(pkey.view()));
                            return reinterpret_cast<Customer *>(
                                const_cast<char *>(
                                    tuple->body_.get_val().data()));
                        }

                        static bool less(const SimpleKey<8> &lh,
                                         const SimpleKey<8> &rh) {
    ...
    static void load_district([[maybe_unused]] Param *param,
                              std::uint16_t w_id) {
        struct S {
            static void work([[maybe_unused]] Param *param, uint8_t d_id,
                ...
                make_address(district.D_STREET_1, district.D_STREET_2,
                ...
                // CREATE Customer History Order Orderline. 3000 customers per a
                // district.
    ...
    static void load(Param *param) {
        ...
        for (std::size_t w = 1; w <= TPCC_NUM_WH; ++w) {
