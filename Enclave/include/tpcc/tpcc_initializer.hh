...
template <typename Tuple, typename Param> class TPCCInitializaer {
  public:
        ...
        struct S {
            static void work([[maybe_unused]] Param *param,
                ...
                for (std::uint32_t i_id = i_id_start; i_id <= i_id_end;
                    ...
                    std::size_t dataLen =
                        random_alpha_string(26, 50, ite.I_DATA);
                    ...
                    db_insert_raw(0, param, Storage::Item, key.view(),
                                  std::move(obj));
                }
            }
        };
...
// #endif
                    ...
                    Tuple *tuple =
                        Table[get_storage(Storage::CustomerSecondary)].get(
                            c_last_key);
                    if (tuple != nullptr) {
                    ...
                    struct S {
                        static Customer *search(const SimpleKey<8> &pkey) {
                            // MasstreeWrapper<Tuple>::thread_init(cached_sched_getcpu());
                            // auto *tuple =
                            // reinterpret_cast<Tuple*>(Masstrees[get_storage(Storage::Customer)].get_value(pkey.view()));
                            auto *tuple = reinterpret_cast<Tuple *>(
                                Table[get_storage(Storage::Customer)].get(
                                    pkey.view()));
    ...
    static void load(Param *param) {
        ...
        {
            load_item(param);
            load_warehouse(param, 1);
            load_stock(param, 1);
            load_district(param, 1);
        }
